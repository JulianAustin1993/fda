...
package object linalg {
   ...
   */
  def cholesky(X: DenseMatrix[Double]): DenseMatrix[Double] = {
   ...
   */
  def eigh(X: DenseMatrix[Double]): (DenseVector[Double], DenseMatrix[Double]) = {
    val A = lowerTriangular[Double](X)
    val N = A.rows
   ...
   */
  def choSolve(L: DenseMatrix[Double], B: DenseMatrix[Double]): DenseMatrix[Double] = {
